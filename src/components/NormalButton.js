...
const Border = styled.div`
  ...
  background-size: calc(100% - (var(--border-size) * 2))
      ...
      cover;
  position: absolute;
  bottom: ${props => `calc(50% - ${props.size / 2}px)`};
  ...
  mask: ${props => `url(${createSVGMask(props.buttonWidth + 4, props.buttonHeight + 4)})`};
  mask-size: ${props => `${props.buttonWidth + 4}px ${props.buttonHeight + 4}px`};
...
function NormalButton({ noStar, children, onClick }) {
  ...
  return(
    <div>
      <ButtonWrapper>
        <Border 
          as={motion.div}
          animate={{'--border-angle': ["0turn", "1turn"]}}
